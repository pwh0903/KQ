...
def get_nums(num_window):
    nums_list = dict()
    imgs = os.listdir('numbers')
    for img in imgs:
        if not img.endswith('png'):
            continue
        file_name, file_extension = os.path.splitext(img)
        template = cv2.imread('numbers/{}'.format(img), 0)
...
if __name__ == '__main__':
    ...
    for img in test_imgs:
        ...
        ret, pot_window = cv2.threshold(pot_window, 100, 255, cv2.THRESH_BINARY)
...
# im = cv2.imread('poker/t.png', 0)
# h, w = im.shape
# pot_window = im[int(148*h/543): int(165*h/543), int(650*w/1580): int(960*w/1580)]
# ret, pot_window = cv2.threshold(im, 100, 255, cv2.THRESH_BINARY)
# nums = get_nums(pot_window)
# print(nums)
